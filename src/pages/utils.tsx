...
const tools = [
  ...
  { title: "RPS Multiplayer", description: "Rock Paper Scissors — play against others in real-time.", href: "/utils/rps", icon: "✊" },
]
...
const Utils = () => {
  return (
    <div class="flex flex-col items-center min-h-[calc(100vh-12rem)] pt-8">
      ...
      <p class="text-stone-400 text-lg sm:text-xl mb-12 text-center">A growing collection of handy tools</p>
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 w-full max-w-4xl pb-5">
      {tools.map((tool) => (
        <A
          href={tool.href}
          class="block rounded-xl border border-stone-200 bg-white p-6 shadow-sm transition hover:shadow-md hover:border-stone-300 hover:-translate-y-0.5"
        >
          <div class="mb-3 text-4xl">{tool.icon}</div>
          <h2 class="text-xl font-semibold mb-1 text-stone-800">{tool.title}</h2>
          <p class="text-stone-500 text-sm">{tool.description}</p>
        </A>
      ))}
      </div>
    </div>
